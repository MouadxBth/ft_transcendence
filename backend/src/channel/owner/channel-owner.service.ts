   ...
   constructor(private readonly prisma : PrismaService) {}
    ...
    async createAdmin(user: User, dto: AdminOperationsDto)
    {
      ...
      if (result.member.admin)
            throw new HttpException("Target user of the operation is already admin!", HttpStatus.BAD_REQUEST);
    ...
    async removeAdmin(user: User, dto: AdminOperationsDto)
    {
      ...
      if (!(result.member.admin))
            throw new HttpException("Target user of the operation is not an admin!", HttpStatus.BAD_REQUEST);
