...
export interface ChannelPromoteFormProps {
	name: string;
}

const ChannelPromoteForm = ({ name }: ChannelPromoteFormProps) => {
	...
	function onSubmit(data: ChannelOperationFormType) {
		channels?.emit("promote_in_channel", {
		...
		<Form {...form}>
			...
			>
				<GenericFormField
					...
					label="Nickname"
					type="text"
					description="Promote any member you want to become an Admin:"
				/>
...
export default ChannelPromoteForm;
