...
export interface ChannelDemoteFormProps {
	name: string;
}

const ChannelDemoteForm = ({ name }: ChannelDemoteFormProps) => {
	...
	function onSubmit(data: ChannelOperationFormType) {
		channels?.emit("demote_in_channel", {
		...
		<Form {...form}>
			...
			>
				<GenericFormField
					...
					label="Nickname"
					type="text"
					description="Demote any Admin you want to become a member:"
				/>
...
export default ChannelDemoteForm;
